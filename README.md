...
the validate category validates a category among the four predefined one's so if a user enters a unknown category or a false one the function catches it from the given category
...
the explain risk expects three inputs using those it explains the dangerousness of the scam, using the category it gets a predefined explanation of the ways that scammers may use to scam in that specific category, using the risk level it determines the riskness of the number to the user and in the end it combines all of these into a sentence and prints it in the end it also contains a part of give advise function[modified like that to safely test the function] that is for the recommended actions to the user
...
I chose to use MockAPI as it offered a free alternative that allowed the database to be shared among users and also simulated a real-world  scenario , The validate functions (validate_phone and validate_category) could have been combined into one function , but i chose to split them for easier testing and readability. The check function was designed to accept international phone numbers rather than specific countries or group of countries as scamming is a global issue.
